                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <div class="row mb-4">
                                    ...
                                    <div class="col-lg-10">
                                        <select class="form-select" data-id="<?php echo $data['room_id']; ?>"
                                            name="data[<?php echo $alias; ?>][room_id]" id="rooms" required>
                                    ...
                                    <div class="col-lg-10">
                                        <select class="select2 form-control" name="data[<?php echo $alias; ?>][node_id]"
                                            ...
                                            <?php foreach($film_list as $k => $v){ 
                                                $selected = $data['node_id'] == $k ? 'selected' : '';
                                            ?>
                                ...
                                <div class="row mb-4">
                                    ...
                                    <div class="col-lg-10">
                                        <input name="data[<?php echo $alias; ?>][time]" class="form-control datedtime"
                                            value="<?php echo date('d-m-Y H:i',$data['hour']);?>" required>
                                    </div>
                                </div>
                                <div class="row mb-4">
                                    <label class="col-form-label col-lg-2">Film</label>
                                    <div class="col-lg-10">
                                        <select class="select2 form-control" name="data[<?php echo $alias; ?>][status]"
                                            id="" required>
                                            <option value="1" <?php echo $data['status']== 1 ? 'selected' :'';?>>
                                                Kích hoạt
                                            </option>
                                            <option value="0" <?php echo $data['status']== 0 ? 'selected' :'';?>>
                                                Không kích hoạt
                                            </option>
                                        </select>
                                    </div>
...
<script>
    ...
    var room_id = $('#rooms').attr('data-id');
    $.ajax({
        headers: {
            'X-CSRF-Token': csrfToken
        },
        url: get_room + '/' + $('#branchs').val(),
        data: {},
        type: 'post',
        dataType: 'html',
        success: function (d) {
            ...
            if (d.res == 'done') {
                ...
                d.data.forEach(element => {
                    var select = room_id == element.id ? 'selected' : '';
                    html += `<option value="${element.id}" ${select}>${element.title}</option>`;
                });
                $('#rooms').html(html);
            } else {
